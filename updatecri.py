...
def play_game(message):
    global current_over, current_ball, current_batsman_ready, current_batsman, innings_count
    ...
    if current_state == STATE_BAT:
        if not current_batsman_ready:
            if message.text.lower() == 'ready':
                current_batsman_ready = True
                bot.send_message(message.chat.id, 'Batsman {} is ready.'.format(message.from_user.username))
                return
            else:
                bot.send_message(message.chat.id, 'Please type "ready" when you are ready to bat.')
                return
...
def switch_roles():
    global current_state, current_batsman, current_bowler, current_batsman_ready
    current_batsman_ready = False
